...
adb root &>/dev/null || { echo -e "Error: Failed to get root access via adb.\n"; exit 1; }

# Pull the boot image:
echo -e "Pulling boot image...\n"
adb pull "$BOOT_PATH" && mv -f boot_* boot.img || { echo -e "Error: Failed to pull or rename boot image.\n"; exit 1; }

# Pull the dtbo image:
echo -e "Pulling dtbo image...\n"
adb pull "$DTBO_PATH" && mv -f dtbo_* dtbo.img || { echo -e "Error: Failed to pull or rename dtbo image.\n"; exit 1; }
